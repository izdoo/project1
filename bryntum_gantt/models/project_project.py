         ...
         calendars_objs = [
                {
                    ...
                    "working_intervals": [
                        {
                            "name": interval.name,
                            "day_period": getattr(interval, "day_period", None),
                            "day_of_week": getattr(interval, "dayofweek", None) or getattr(interval, "day_of_week", None),
                            "resource_id": interval.resource_id.id if interval.resource_id else False,
                            "resource_name": interval.resource_id.name if interval.resource_id else "",
                    ...
                    "leave_intervals": [
                        {
                            "name": interval.name,
                            "time_type": getattr(interval, "time_type", None),
                            "resource_id": interval.resource_id.id if interval.resource_id else False,
                            "resource_name": interval.resource_id.name if interval.resource_id else "",
                            "date_from": get_gantt_date(interval.date_from),
                            "date_to": get_gantt_date(interval.date_to)
                        }
                        for interval in getattr(calendar, "leave_ids", [])
                    ]
                }
